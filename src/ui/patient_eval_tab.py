...
def render_patient_eval_tab(*, patient_evaluator: Any) -> None:
    logger.info("Rendering patient evaluation tab")
    
    st.subheader("Patient evaluation")

    if not getattr(patient_evaluator, "available", True):
        logger.warning("DeepEval not available in patient evaluation tab")
        st.warning("DeepEval not installed. Install with: `pip install -U deepeval`")
        return

    if not os.getenv("OPENAI_API_KEY"):
        logger.warning("OPENAI_API_KEY missing in patient evaluation tab")
    ...
    if st.button("Run patient evaluation"):
        logger.info(f"Patient evaluation triggered: role_threshold={role_threshold}, geval_threshold={geval_threshold}")
        ...
        try:
            ...
            logger.info(f"Patient evaluation completed: {len(out.get('metrics', []))} metrics")
